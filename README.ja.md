...
- **深さのカスタマイズ**: どの見出しレベルまで番号を振るかを指定できます（デフォルトは `6`。Markdownの最大レベルであるため、実質的にすべての見出しが対象になります）。
...
- **型**: `number`
- **デフォルト**: `6`
- **説明**: 見出しの階層をどこまでチェックするかを指定します。Markdownの仕様では見出しはレベル6（`######`）が最大であるため、デフォルトの `6` は「すべての階層の見出しをチェック対象とする」ことを意味します。
- **例**: ドキュメントが `##` から始まり `depth` が `2` の場合、`##` と `###` がチェック対象になります。
